...
Se vuelven a observar valores extremos por encima del bigotes superior del gráfico. Como en el caso anterior es totalmente posible que viajen personas por encima de 60 años. Por esta ranzón y para no perder información valiosa, dejaremos la variable sin modificar en el dataset. 
...
Antes de realizar la discretación, mostramos el histograma de la variable para comprender mejor cómo se distribuye.

```{r}
hist(titanic_data$Age)
```

Lo primero que se observa es que gran parte de los datos se concentran entre 20 y 40. Lo que nos indica que los tres niveles que queremos construir tendrán un número de registros muy desigual. Pero igualmente nuestro objetivo con la discretación es estudiar si se cumple la máxima "Mujeres, niños y ancianos primero" cuando se produce una accidente marítimo.
...
La precisión que nos da el modelo es de 78,79 %. Es decir, el 78,79 % de los registros han sido correctamente clasificados.
...
Con el modelo Random Forest el 81,48 % de los registros de test fueron correctamente clasificados.La precisión de este modelo ha aumentado casi 3% respecto al árbol de decisión.

La sensibilidad de este modelo es 60,53 % y su especificidad 94,54 %. Si comparamos estos valores con los obtenidos mediante el árbol de decisión, se obtiene que el modelo Random Forest clasifica igual de bien (o mal) los pasajeros que no sobrevivieron, pero en cambio, los pasajeros que sí sobrevivieron son clasificados correctamente con un porcentaje bastante mayor.
...
En todas las edades se observa una gran desigualdad entre el índice de supervivencia si se comparan hombres con mujeres. Destaca que las todas mujeres por encima de 60 años que viajaban en el barco se salvaron. Aunque según vimos en la distribución de los datos es posible que no fueran muchas. En cambio, la tasa de supervivencia de los hombres disminuye según avanza el grupo de edad. Aproximadametne el 40% de los niños de sexo masculino sobreviven al accidente, mientras que el porcentaje de ancianos hombres que sobrevieron al accidente disminuye a la mitad, un 20%.

Por lo tanto, todo hace indicar que priorizaron a mujeres y niños a la hora de subir a los botes salvavidas. Como suele pasar en cualquier tipo de accidente marítimo. Respecto a los pasajeros por encima de 60 años, solo podemos afirmar que ningunga mujer murió en ese rango de edad, como ya se ha comentado.
