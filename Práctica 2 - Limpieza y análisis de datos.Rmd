...
Existen dos variables que continen valores vacíos: la variable Age con 263 NAs y la variable Fare con 1 NA.
...
En el caso del atributo Fare, existe únicamente un valor vacío. Como en el caso anterior, una posibilidad sería eleminar el registro en cuestión. Pero obtaremos por imputar dicho valor para no perder información. Para ello utilizaremos la mediana. En lugar de calcular la mediana de todos los datos disponibles, calcularemos la mediana de los registros que pertenezcan a la misma clase. Como veremos en el apartado donde se tratan los outliers, la dispersión de la variable Fare es grande. Así que tiene sentido centrarse solo en una clase, ya que los precios dentro de ella puede que varíen menos.

Vemos en que clase se encuentra el valor perdido.

```{r}
titanic_data$Pclass[is.na(titanic_data$Fare)]
```

Sustituimos el valor por la mediana de los valores de esa clase.

```{r}
titanic_data$Fare[is.na(titanic_data$Fare)] <- median(titanic_data$Fare[titanic_data$Pclass == 3], na.rm = TRUE)
```

En la variable Cabin el número de elementos vacíos es muy elevado, 1014 de un total de 1309 registros (77,1%). Debido a este alto porcentaje de elementos vacíos, se sustituyen las cadenas vacías con la constante "Desconocido". Otra posibilidad podría haber sido eliminar por completo la variable. Esta segunda aproximación también tiene sentido debido al elevado número de valores vacíos y a que el atributo Pclass nos puede dar una información similar de cara a saber si un pasajero sobrevive o no. Los camarotes de primera clase estarían situados cerca de cubierta, por la probabilidad de sobrevivir sería mayor que la de un pasejero que viaje en tercera clase y cuyo camarote se encuentre en una planta muy inferior.
...
Se observan valores extremos tanto por encima, como por debajo de los bigotes del gráfico. En este caso es totalmente posible que viajen personas por encima de 60 años y recién nacidos en el barco. Una posible sería no hacer nada con estos outliers para no perder información. En su lugar, optaremos por discretizar la variable. Se opta por crear cinco niveles de frecuencia parecida. Mostramos el histograma del atributo para ver cómo se distribuye. Mostramos agrupaciones de dos en dos, para obaservarlo con más detalle.
...
Al existir un total de 1309 registros en el conjunto de datos y querer discretizar la edad en 5 niveles, el objetivo será obtener aproximadametne 260 registros en cada nivel. observando el histograma y probando diferetentes límites, los cinco niveles de discretización serán los siguientes:

* Joven: [0,20]
* Joven-Adulto: [21,24]
* Adulto: [25,28]
* Adulto-Mayor: [29,39] (Tras jugar un poco con los valores se ha visto que se reparte la densidad en los niveles con 39 que con 40).
* Mayor: [40,80]
...
```{r}
titanic_data$Age <- cut(titanic_data$Age, breaks = c(0,20,24,28,39,80), ordered_result = TRUE, labels = c("Joven", "Joven-Adulto", "Adulto", "Adulto-Mayor","Mayor"))
```
